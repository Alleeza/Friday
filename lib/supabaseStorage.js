...
const gamificationProgressTable = 'gamification_progress';
...
export async function loadPublishedProject(projectId, shareId) {
  const rows = await supabaseRequest(
    `/${publishedProjectsTable}?project_id=${createFilter(projectId)}&share_id=${createFilter(shareId)}&select=project_id,project_json,source_updated_at,published_at`,
  );
...
export async function loadGamificationProgress(userId) {
  const rows = await supabaseRequest(
    `/${gamificationProgressTable}?user_id=${createFilter(userId)}&select=progress_json,updated_at`,
  );
...
export async function saveGamificationProgress(userId, progress) {
  const updatedAt = new Date().toISOString();
  const rows = await supabaseRequest(`/${gamificationProgressTable}?on_conflict=user_id`, {
    method: 'POST',
    prefer: 'resolution=merge-duplicates,return=representation',
    body: [{
      user_id: userId,
  ...
  const row = rows?.[0];
  return {
    progress: row?.progress_json || progress,
    updatedAt: row?.updated_at || updatedAt,
  };
}
