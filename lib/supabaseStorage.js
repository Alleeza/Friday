...
async function supabaseRequest(pathname, { method = 'GET', body = null, prefer = '' } = {}) {
  try {
    ...
    throw attachDiagnostic(error, {
      step: error?.diagnostic?.step || 'supabase.request',
      method,
      pathname,
    });
  }
...
export async function loadProject(projectId) {
  ...
  try {
    ...
    throw attachDiagnostic(error, { step: 'editor.load', projectId });
  }
...
export async function saveProject(projectId, projectState) {
  ...
  let rows;
  try {
    rows = await supabaseRequest(`/${editorProjectsTable}?on_conflict=id`, {
      method: 'POST',
    ...
    throw attachDiagnostic(error, { step: 'editor.save', projectId });
  }
...
export async function publishProject(projectId) {
  let savedProject;
  try {
    savedProject = await loadProject(projectId);
  } catch (error) {
    throw attachDiagnostic(error, { step: 'publish.load-source', projectId });
  }

  if (!savedProject) {
    throw createDiagnosticError(
      'Save the project before generating a share link.',
      { step: 'publish.load-source', projectId, sourceProjectFound: false },
      400,
    );
  }
  ...
  try {
    existingRows = await supabaseRequest(
      `/${publishedProjectsTable}?project_id=${createFilter(projectId)}&select=share_id`,
    );
  } catch (error) {
    throw attachDiagnostic(error, { step: 'publish.lookup-existing', projectId });
  }

  const shareId = existingRows?.[0]?.share_id || crypto.randomBytes(9).toString('base64url');
  const publishedAt = new Date().toISOString();
  const normalizedProject = normalizeProjectState(savedProject);

  let rows;
  try {
    rows = await supabaseRequest(`/${publishedProjectsTable}?on_conflict=project_id`, {
      method: 'POST',
      prefer: 'resolution=merge-duplicates,return=representation',
      body: [{
    ...
    throw attachDiagnostic(error, { step: 'publish.upsert', projectId, shareId });
  }
...
export async function loadPublishedProject(shareId) {
  ...
  try {
    ...
    throw attachDiagnostic(error, { step: 'published.load', shareId });
  }
